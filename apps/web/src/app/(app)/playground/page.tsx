      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="text-xs text-black/45">Structured generation tester</span>
          </div>
          <h1 className="mt-3 text-2xl font-bold tracking-tight text-black">Generate Playground</h1>
          <p className="mt-1 text-sm text-black/55">Test OneAI tasks, model routing, usage, and traces before external callers use them.</p>
        </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid gap-3 md:grid-cols-4">
                <Stat label="Request ID" value={result?.requestId ? <code>{String(result.requestId).slice(0, 8)}</code> : "-"} />
                <Stat label="Attempts" value={result?.attempts ?? "-"} />
                <Stat label="Latency" value={result?.latencyMs ? `${result.latencyMs}ms` : "-"} />
                <Stat label="Cost" value={result?.usageTotal?.estimatedCostUSD ? `$${Number(result.usageTotal.estimatedCostUSD).toFixed(6)}` : "-"} />
              </div>

              {result?.usage ? (
                <div className="grid gap-3 md:grid-cols-3">
                  <Stat label="Provider" value={result.usage.provider || "-"} />
                  <Stat label="Model" value={<code className="break-all">{result.usage.model || "-"}</code>} />
                  <Stat label="Tokens" value={result.usage.totalTokens ?? "-"} />
                </div>
              ) : null}

              {result?.llmTrace?.attempts?.length ? (
                <div className="rounded-lg border border-black/10 bg-white p-3">
                  <div className="mb-2 text-xs font-semibold text-black/55">LLM Trace</div>
                  <div className="space-y-2">
                    {result.llmTrace.attempts.map((attempt: any, index: number) => (
